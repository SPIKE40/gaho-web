...
const JSONViewer = ({ data, selectedDocIndex, selectedPageIndex, setData, onPolygonSelect, onClassSelect }) => {
  ...
  const handleDownload = () => {
    // 원본 데이터를 깊은 복사
    const rawData = JSON.parse(JSON.stringify(data.docs[selectedDocIndex]));
  
    // pages 배열을 순회하며 analyzeResult 아래 content 제거
    if (rawData.pages && Array.isArray(rawData.pages)) {
      rawData.pages.forEach(page => {
        if (page.analyzeResult && page.analyzeResult.hasOwnProperty('content')) {
          delete page.analyzeResult.content;
        }
      });
    }
  
    // 모든 레벨에서 제외할 키 목록 (createdDataTime으로 수정)
    const excludeKeys = [
      'html',
      'markdown',
      'width',
      ...
      'page_image_urls',
      'status',
      'createdDataTime', // 오타 수정: createdDateTime -> createdDataTime
      'lastUpdatedDateTime'
    ];
  
    // 나머지 키는 모든 레벨에서 제외
    const filteredData = JSON.parse(JSON.stringify(rawData, (key, value) => {
    ...
    <div className="json-viewer" style={{ position: 'relative' }} ref={viewerRef}>
      ...
      <div className="download-button-container">
        <button className="download-button" onClick={handleDownload} title="내 컴퓨터 다운로드 폴더에 JSON 파일을 내려받음">
          <FontAwesomeIcon icon={faDownload} /> Layout
        </button>
      </div>
      ...
      {editingPath && (
        <div
          ...
          style={{
            position: 'absolute',
            top: editSectionPosition.y + 70, // "Edit" 버튼 아래 65px 위치
