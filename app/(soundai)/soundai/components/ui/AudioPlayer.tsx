...
export default function AudioPlayer({
  ...
  useEffect(() => {
    ...
    const handleShowAudioPlayer = (event: CustomEvent) => {
      ...
      const newAudioInfo: AudioInfo = {
        ...
        avatarUrl: detail.avatarUrl || "/speakerdata/profiles/man.png",
        dialogueItems: detail.dialogueItems || [],
        isDialogueMode: detail.isDialogueMode || false,
      };
    ...
    window.addEventListener(
      "showAudioPlayer",
      handleShowAudioPlayer as EventListener
    );
  ...
  }, [audioInfo.audioUrl, audioInfo.voiceName]);
  ...
  return (
    <>
      ...
      <div
        className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40 w-3/4 max-w-5xl
      ...
      >
        <div
          className="bg-gradient-to-r from-[#1e1e2f] via-[#2d2d44] to-[#1e1e2f] 
        ...
        >
          ...
          <div className="flex flex-col space-y-5">
              ...
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 rounded-xl overflow-hidden bg-gradient-to-tr from-purple-600 to-blue-400 p-0.5 shadow-lg">
                  <div className="w-full h-full bg-[#1a1a2e] rounded-lg overflow-hidden flex items-center justify-center">
                    <Image
                      src={
                        audioInfo.avatarUrl || "/speakerdata/profiles/man.png"
                      }
                      width={44}
                      height={44}
                      alt="Voice avatar"
                ...
                <div className="text-white">
                  ...
                  {audioInfo.speed && audioInfo.speed !== 1.0 && (
                    <div className="text-xs text-indigo-300 font-medium">
                      재생 속도: {audioInfo.speed.toFixed(2)}x
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* 텍스트 표시 영역 */}
            {audioInfo.isDialogueMode &&
            audioInfo.dialogueItems &&
            audioInfo.dialogueItems.length > 0 ? (
              <div className="bg-[#1a1a2e]/60 p-4 rounded-xl text-sm text-gray-300 max-h-60 overflow-auto scrollbar-thin scrollbar-thumb-gray-600 backdrop-blur-sm border border-[#ffffff10]">
                {audioInfo.dialogueItems.map((item, index) => (
                  <div key={index} className="flex items-start mb-3 last:mb-0">
                    <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-tr from-purple-600 to-blue-400 p-0.5 flex-shrink-0 mr-3">
                      <div className="w-full h-full bg-[#1a1a2e] rounded-full overflow-hidden flex items-center justify-center">
                        <Image
                          src={
                            item.avatarUrl || "/speakerdata/profiles/man.png"
                          }
                          width={32}
                          height={32}
                          alt={`${item.speakerName} avatar`}
                          className="object-cover"
                        />
                      </div>
                    </div>
                    <div className="flex-1">
                      <div className="text-indigo-400 text-xs font-medium mb-1">
                        {item.speakerName}
                      </div>
                      <div className="text-gray-300 bg-[#1a1a2e]/60 p-2 rounded-lg">
                        {item.text}
                      </div>
                    </div>
                  </div>
              ...
              audioInfo.text && (
                <div className="bg-[#1a1a2e]/60 p-4 rounded-xl text-sm text-gray-300 max-h-32 overflow-auto scrollbar-thin scrollbar-thumb-gray-600 backdrop-blur-sm border border-[#ffffff10]">
                  <span className="text-indigo-400 text-xl opacity-50">
                    &quot;
                  </span>
                  {audioInfo.text}
                  <span className="text-indigo-400 text-xl opacity-50">
            ...
            <div className="bg-[#1a1a2e]/60 p-5 rounded-xl flex items-center space-x-4 backdrop-blur-sm border border-[#ffffff10]">
              ...
              >
                {isPlaying ? (
                  <PauseCircleOutlined className="text-indigo-400 hover:text-indigo-300" />
                ) : (
                  <PlayCircleOutlined className="text-indigo-400 hover:text-indigo-300" />
                )}
              </button>
              ...
              <div className="flex-grow mx-3">
                <Slider
                  ...
                  styles={{
                    track: {
                      background: "linear-gradient(to right, #8a2be2, #4158D0)",
                    },
                    handle: {
                      borderColor: "#8a2be2",
                      backgroundColor: "#fff",
